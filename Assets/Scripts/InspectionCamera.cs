...
public class InspectionCamera : MonoBehaviour
{
    ...
    private void RotateObject()
    {
        ...
        {
            _objectRotator.Rotate(new Vector3(Input.GetAxis("Mouse Y"), -Input.GetAxis("Mouse X"), 0) * Time.deltaTime * _rotationSpeed,
                Space.World);
        }
        if (Input.GetKey(KeyCode.A))
        {
            _objectRotator.Rotate(new Vector3(0, 0, 0.13f) * Time.deltaTime * _rotationSpeed,
                Space.World);
        }
